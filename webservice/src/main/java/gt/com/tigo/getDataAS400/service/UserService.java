...
import gt.com.tigo.getDataAS400.dao.resourceManager.ApplicationParameterRepository;
import gt.com.tigo.getDataAS400.dao.resourceManager.GroupRepository;
...
import gt.com.tigo.getDataAS400.dao.resourceManager.UserRepository;
import gt.com.tigo.getDataAS400.dto.FilterInfoDto;
import gt.com.tigo.getDataAS400.util.SpecFactory;
...
import static gt.com.tigo.getDataAS400.util.SpecFactory.*;
...
public class UserService extends AbstractService<AdmUsuarioEntity, UserRepository> {
    ...
    public AdmUsuarioEntity findByUid(String token) throws ResourceNotFoundException, ResourceCreateException, ResourceUpdateException {
        ...
        if (user == null) {
            try {
                ...
                // se vuelve a consultar el nuevo usuario para obtener el grupo asignado
                try {
                    LOGGER.debug(String.format("Searching groups for user with email %s", userInfo.get(MAIL).toString()));
                    user.setGrupos(this.groupRepository.findDefaultGroup());
                } catch (Exception ex) {
                    LOGGER.error(ex.getMessage());
                    throw new ResourceNotFoundException();
                }
    ...
    protected Specification buildFilterSpecification(DataTableRequestDto dataTableRequestDto) throws InvalidFilterException {
        ...
        try {
            ...
            Specification filterSpec = Specification
